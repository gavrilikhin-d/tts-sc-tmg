...
export type Size<S extends State> = S extends "Raw"
  ? `${number}`
  : Distinct<"Size", number>;
export type Inches = Distinct<"Inches", number>;
export type Speed<S extends State> = S extends "Raw"
  ? `${Inches}` | `${Inches}/${Inches}`
  : {
      multipleModels: Inches;
      singleModel: Inches;
    };
export type Roll<S extends State> = S extends "Raw"
  ? `${2 | 3 | 4 | 5 | 6}+`
  : Distinct<"Roll", 2 | 3 | 4 | 5 | 6>;
export type HP<S extends State> = S extends "Raw"
  ? `${number}`
  : Distinct<"HP", number>;
export type Shield<S extends State> = S extends "Raw"
  ? `${number}`
  : Distinct<"Shield", number>;
...
export type SurgeDescription =
  `${SurgeType | `${SurgeType}, ${SurgeType}`} (${Die})`;
export type RoA = Distinct<"RoA", number>;
export type DiceAmount = Distinct<"DiceAmount", number>;
export type WeaponDescription<S extends State> = S extends "Raw"
  ? `RANGE: ${Inches | "E"} | TARGET: ${Target} | RoA: ${RoA} | HIT: ${Roll<"Raw">} | DMG: ${number}\nSURGE: ${SurgeDescription | "-"}`
  : {
      range: Inches;
      target: Target;
      roa: RoA;
...
export type Activation<S extends State> = S extends "Raw"
...
export type UpgradeDescription<S extends State> = S extends "Raw"
  ? string | WeaponDescription<"Raw">
  : string | WeaponDescription<"Parsed">;
export type LinkedTo<S extends State> = S extends "Raw"
  ? "" | "-" | UpgradeName
...
export type Upgrade<S extends State> = {
  name: UpgradeName;
  linkedTo: LinkedTo<S>;
  activation: S extends "Raw"
    ? Activation<"Raw"> | ""
    : Activation<"Parsed"> | undefined;
