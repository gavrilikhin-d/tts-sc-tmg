...
const postprocessAbility = (ability: Ability<"Raw">): Ability<"Parsed"> => {
...
const postprocessSquadProfile = ({
  modelCount,
  ...rest
}: SquadProfile<"Raw">): SquadProfile<"Parsed"> => {
  if (modelCount === "-") {
    return {
      ...rest,
...
const postprocessUnitStats = (stats: UnitStats<"Raw">): UnitStats<"Parsed"> => {
  return {
    size:
      stats.size === "-" ? undefined : (parseInt(stats.size) as Size<"Parsed">),
    ...
    shield:
      stats.shield === "-"
        ? undefined
        : (parseInt(stats.shield) as Shield<"Parsed">),
  };
};

const postprocessActivation = (
...
const postprocessUpgradeDescription = (
  description: UpgradeDescription<"Raw">,
): UpgradeDescription<"Parsed"> => {
  ...
  return {
    ...
    surge:
      surge.trim() === "-"
        ...
        : {
            against: (surge
              .match(/^(Light|Armoured)(?:,\s*(Light|Armoured))?/)
              ?.slice(1)
              .filter(Boolean) ?? []) as SurgeType[],
            die: (surge.match(/\((D3|D6)(?:\+\d+)?\)/)?.[1] ?? "D3") as Die,
            plus: parseInt(
              surge.match(/\((?:D3|D6)\+(\d+)\)/)?.[1] ?? "0",
            ) as DiceAmount,
          },
...
export const postprocessUpgrade = (
  upgrade: Upgrade<"Raw">,
): Upgrade<"Parsed"> => {
  const phase = upgrade.phase.match(
    /^(Movement|Assault|Combat|Any) Phase$/,
  )?.[1];
  if (!phase) {
  ...
  return {
    ...
    activation: upgrade.activation
      ? postprocessActivation(upgrade.activation)
      : undefined,
    phase: phase as Phase,
  };
};

export const postprocessUnitCard = (
  card: UnitCard<"Raw">,
): UnitCard<"Parsed"> => {
  return {
    ...card,
