...
export const postprocessAbility = (ability: Ability<"Raw">): Ability<"Parsed"> => {
    ...
    const match = prefix.match(/.* <(Active|Passive|Reaction)> <(Movement|Assault|Combat|Any) Phase>/);
...
export const postprocessTacticalCards = (cards: TacticalCard<"Raw">[]): TacticalCard<"Parsed">[] => {
    return cards.map(card => ({ ...card, boosts: card.boosts.map(postprocessAbility) }));
};