...
lowerUpperBtn={
    ...
    position={-5.62, Yoffset ,1.1}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
...
lethalHitsBtn={
    ...
    position={lethalButtonsOffset, Yoffset ,1.1}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color="White", font_color={0,0,0}
}
...
sustainedHitsBtn={
    ...
    position={lethalButtonsOffset + 0.7, Yoffset ,1.1}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color="White", font_color={0,0,0}
}
...
sustainedLethalHitsBtn = {
    ...
    position={lethalButtonsOffset + 1.61, Yoffset ,1.1}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color="White", font_color={0,0,0}
}
copyOthersDiceBtn = {
...
gap2Btn={
    label='1" Gap', tooltip='Arrange models with 1" spacing', click_function='arrangeModelsWith1Inch', function_owner=self,
    position={-7.7, Yoffset ,-1.15}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

gap1Btn={
    label='2" Gap', tooltip='Arrange models with 2" spacing', click_function='arrangeModelsWith2Inch', function_owner=self,
    position={-7.15, Yoffset ,-1.15}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

clearMeasureAllBtn={
    label='Clear Bubbles', tooltip='Clear all measuring bubbles on the table', click_function='clearMeasuringCircles', function_owner=self,
    position={-6.325, Yoffset ,-1.15}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
...
measure1Btn={
    ...
    position={-7.82, Yoffset ,-1.40}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

measure3Btn={
    label='3"', tooltip='Toggle 3" bubble', click_function='toggle3Bubble', function_owner=self,
    position={-7.52, Yoffset ,-1.40}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

measure6Btn={
    label='6"', tooltip='Toggle 6" bubble', click_function='toggle6Bubble', function_owner=self,
    position={-7.22, Yoffset ,-1.40}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

measure9Btn={
    label='9"', tooltip='Toggle 9" bubble', click_function='toggle9Bubble', function_owner=self,
    position={-6.92, Yoffset ,-1.40}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}

measure12Btn={
    label='12"', tooltip='Toggle 12" bubble', click_function='toggle12Bubble', function_owner=self,
    position={-6.59, Yoffset ,-1.40}, rotation={0,0,0}, height=0, width=0, scale={0,0,0},
    font_size=0, color='White', font_color={0,0,0}
}
...
customBubbleInputBG = {
    ...
    position = {-6.27, Yoffset, -1.40}, rotation = {0, 0, 0},
    width = 0, height = 0, font_size = 0,
...
customBubbleInput = {
    ...
    position = {-6.27, Yoffset, -1.40}, rotation = {0, 0, 0},
    width = 0, height = 0, font_size = 0, alignment = 3, scale = {0, 0, 0},
    color = {1, 1, 1, 1}, font_color = {0.2, 0.2, 0.2}
}

-- Custom Bubble Button
customBubbleBtn = {
    label = "Go", tooltip = "Apply custom bubble size", click_function = "applyCustomBubble", function_owner = self,
    position = {-6.07, Yoffset, -1.40}, rotation = {0, 0, 0},
    width = 0, height = 0, font_size = 0,
...
local redoRollBtn = {
    label          = "Redo Roll",
    tooltip        = "Redo previously reverted roll",
    click_function = "redoRoll",
    function_owner = self,
    position       = {-6.0, Yoffset, -0.16},
    rotation       = { 0, 0, 0 },
    width = 800, height = 300, font_size = 120,
...
function redoRoll(obj, player, alt)
    -- Move forward in the previousRollStack if possible
    if not previousRollStack or #previousRollStack == 0 then
        printToColor("No roll history available.", player, "Red")
        return
    end
    if previousRollIndex >= #previousRollStack then
        printToColor("No newer roll to redo.", player, "Red")
        return
    end
    previousRollIndex = previousRollIndex + 1
    local snapshot = previousRollStack[previousRollIndex]
    if not snapshot or #snapshot == 0 then
        printToColor("No roll data at this index.", player, "Red")
        return
    end
    for _, die in ipairs(getDice()) do die.destroy() end
    for _, data in ipairs(snapshot) do
    ...
    printToColor("Redone to roll #" .. tostring(previousRollIndex) .. " of " .. tostring(#previousRollStack), player, "Green")
    previousRollData = previousRollStack[previousRollIndex] or {}
end
...
local function evalSimpleExpr(expr)
  if type(expr) ~= "string" then return nil, "invalid input" end
  local s = expr:gsub("[xX]", "*")
  s = s:gsub("%s+", "")
  if s == "" then return nil, "empty" end
  -- allow only digits, operators and parentheses
  if s:find("[^%d%+%-%*/%(%)]") then return nil, "invalid characters" end
  ...
  while i <= n do
    ...
    elseif c == '-' then
      ...
      local prevIsOp = (prev == nil) or (prev == '+') or (prev == '-') or (prev == '*') or (prev == '/') or (prev == '(')
      if prevIsOp then table.insert(tokens, 0) end
      table.insert(tokens, '-')
      i = i + 1
  ...
  local output, ops = {}, {}
  local prec = { ['+'] = 1, ['-'] = 1, ['*'] = 2, ['/'] = 2 }
  local function isop(x) return prec[x] ~= nil end
  for _, t in ipairs(tokens) do
    ...
    elseif t == ')' then
      ...
      while #ops > 0 do
        local top = ops[#ops]
        table.remove(ops)
        if top == '(' then found = true break end
        table.insert(output, top)
      end
      if not found then return nil, "mismatched parentheses" end
    elseif isop(t) then
  ...
  while #ops > 0 do
    local top = ops[#ops]
    table.remove(ops)
    if top == '(' then return nil, "mismatched parentheses" end
    table.insert(output, top)
  end
  ...
  for _, t in ipairs(output) do
    if type(t) == 'number' then
      ...
      local a, b = pop2()
      if a == nil then return nil, "invalid expression" end
      if t == '+' then table.insert(stack, a + b)
      elseif t == '-' then table.insert(stack, a - b)
      elseif t == '*' then table.insert(stack, a * b)
      elseif t == '/' then
  ...
  if #stack ~= 1 then return nil, "invalid expression" end
  return stack[1]
end
...
function getDice()
    ...
    local lethalGUID = (currentMat == "Red")
        and Global.getVar("redLethalHits_GUID")
        or   Global.getVar("blueLethalHits_GUID")
    local lzone = getObjectFromGUID(lethalGUID)
    local lbound
    if lzone then
        local lb = lzone.getBounds()
        lbound = {
            minx = lb.center.x - lb.size.x/2,
            maxx = lb.center.x + lb.size.x/2,
            minz = lb.center.z - lb.size.z/2,
            maxz = lb.center.z + lb.size.z/2,
            miny = lb.center.y - lb.size.y/2,
            maxy = lb.center.y + lb.size.y/2
        }
    end
    ...
    for _, o in ipairs(getAllObjects()) do
        if o.tag == "Dice" and o.resting then
            ...
            and not (lbound
                and p.x > lbound.minx and p.x < lbound.maxx
                and p.z > lbound.minz and p.z < lbound.maxz
                and p.y > lbound.miny and p.y < lbound.maxy)
            then
                table.insert(dice, o)
            end
        end
...
function sustainedHitsAction(_, color, alt)
    ...
    -- Helper: compare positions (x,z) with small epsilon
    local function near_pos(a, b, eps)
        eps = eps or 0.5
    ...
    -- Helper: compare color tint approximately
    local function near_tint(a, b, eps)
        eps = eps or 0.05
        if not a or not b then return false end
        return math.abs(a[1]-b[1]) <= eps and math.abs(a[2]-b[2]) <= eps and math.abs(a[3]-b[3]) <= eps
    end

    -- Match previous roll entries to current dice by value + tint + approximate position
    if previousRollData and #previousRollData > 0 then
        local prevCandidates = {}
        for _, d in ipairs(previousRollData) do
            if d.val >= threshold then
                table.insert(prevCandidates, d)
            end
        end

        if #prevCandidates > 0 then
            local currentDice = getDice()
            local used = {}
            for _, prev in ipairs(prevCandidates) do
                for i, die in ipairs(currentDice) do
                    if not used[i] and die.getValue() == prev.val then
                        local tint_ok = near_tint(die.getColorTint(), prev.tint)
                        local pos_ok = near_pos(die.getPosition(), prev.pos)
                        if tint_ok and pos_ok then
    ...
    -- Fallback: if we didn't collect any by roll-matching, fall back to global behavior
    if #diceToClone == 0 then
        for _, die in ipairs(getDice()) do
            if die.getValue() >= threshold then
                table.insert(diceToClone, die)
            end
        end
        printToAll("Warning: No previous roll match found â€” duplicated all current dice >= " .. threshold .. "+", {1,0.5,0.2})
    end
...
function sustainedLethalHitsAction(_, color, alt)
    ...
    -- Fallback to global selection if none matched
    if #sixDice == 0 then
        for _, die in ipairs(getDice()) do
            if die.getValue() >= threshold then
                table.insert(sixDice, die)
    ...
    -- Move lethal hits using same left/right semantics
...
function onload()
    ...
    self.createButton(clearMeasureAllBtn)
    self.createButton(gap2Btn)
    self.createButton(gap1Btn)
...
local lethalHitsImageURL = "https://steamusercontent-a.akamaihd.net/ugc/10593277004116480902/D0CC2FC9091B2A05F33B6C76BD78E9C2EEAA62C8/"
    ...
    if currentMat == "Red" then
        if not spawnedObjects.Red.lethalHits then
            ...
            if(Global.getVar("redLethalFace")) then
                ...
                redLethalHitsObject.setDescription("RaulDeFama Week!! Special thanks for creating 4k versions of the Letahl Hits Face!") 
    ...
    elseif currentMat == "Blue" then
        if not spawnedObjects.Blue.lethalHits then
            ...
            if(Global.getVar("blueLethalFace")) then
                ...
                blueLethalHitsObject.setDescription("RaulDeFama Week!! Special thanks for creating 4k versions of the Letahl Hits Face!") 
...
function lethalHitsAction(lethalHits, color, alt)
    ...
    -- Set threshold based on click: left-click = 6+, right-click = 5+
    ...
    -- Log for players how many lethal/sustained were moved
...
function applyCustomBubble(_, player, _)
    if customBubbleSize > 0 then
        toggleMeasurementCircleForSelection(customBubbleSize, player)
    else
        printToColor("Please enter a valid bubble size", player, "Red")
    end
end
...
function clearMeasuringCircles(obj, playerColor, alt)
    -- Always perform an aggressive clear (clear all vector lines/tables on Global, this mat and all objects)
    local clearedAll = 0
    if Global and Global.setVectorLines then
        Global.setVectorLines({})
        if Global.setTable then Global.setTable("ym-measuring-circles", nil) end
    end
    if self and self.setVectorLines then
        self.setVectorLines({})
        if self.setTable then self.setTable("ym-measuring-circles", nil) end
    end
    for _, o in ipairs(getAllObjects()) do
        if o and o.setVectorLines then
            o.setVectorLines({})
            if o.setTable then o.setTable("ym-measuring-circles", nil) end
            clearedAll = clearedAll + 1
        end
    end
    printToAll("Cleared vector lines on "..clearedAll.." objects.", {1,1,1})
end